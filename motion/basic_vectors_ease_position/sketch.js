...
let easeFunction = "easeInOutSine"
...
function draw() {
  ...
  // Use normalized position as input to easing function
  let normPos = position.x / width;
  let easeVal;
  switch(easeFunction){
    case "easeInSine":
      easeVal = easeInSine(normPos);
      break;
    case "easeInOutSine":
      easeVal = easeInOutSine(normPos);
      break;
    case "easeOutSine":
      easeVal = easeOutSine(normPos);
      break;
    case "easeInExpo":
      easeVal = easeInExpo(normPos);
      break;
    case "easeOutExpo":
      easeVal = easeOutExpo(normPos);
      break;
    case "easeInOutExpo":
      easeVal = easeInOutExpo(normPos);
      break;
  }

  // Use easeVal to set velocity, scaled for reasonable speed
  let velocity = 2 + 8 * easeVal; // base speed + scaled ease
  position.x += velocity;
