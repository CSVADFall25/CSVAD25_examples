...
public class DShape implements DObj{
	...
	public ArrayList<DPoint> verticies;
	...
	private PApplet myParent;
	  ...
	  DShape(PApplet myParent,DColor fillColor, DColor strokeColor, float strokeWeight,boolean noFill, boolean noStroke){
	   ...
	   this.verticies = new ArrayList<DPoint>();
	  ...
	  public void pushMatrix(){
		...
		public DPoint calculateCentroid(){
			...
			for(int i = 0;i<this.verticies.size();i++){
				xSum+= this.verticies.get(i).x;
				ySum+= this.verticies.get(i).y;
				
			}
			
			return new DPoint(xSum/this.verticies.size(),ySum/this.verticies.size());	
		}
	...
	public ArrayList<DPoint> getNormalizedVertices(){
		...
		for(int i = 0;i<this.verticies.size();i++){
			float nX = this.verticies.get(i).x-centroid.x;
			float nY = this.verticies.get(i).y-centroid.y;
